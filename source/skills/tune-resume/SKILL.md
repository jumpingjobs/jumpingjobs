...
8. Copy the base resume to a new HTML file. The filename MUST match the job posting Markdown
   filename **character-for-character** but with an `.html` extension, saved in the **same
   directory as the job posting** — copy the posting's actual filename, never re-derive the
   slug from the job title. For example, if the posting is
   `job-postings/1-scraped/20260606-acme-staff-engineer.md`, the tuned resume is
   `job-postings/1-scraped/20260606-acme-staff-engineer.html`. The shared prefix is what
   keeps a posting's files grouped through every pipeline move.
